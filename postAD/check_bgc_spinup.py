...
def _annual_means_by_year(ds, default_year=None):
    ...
    if "time" not in ds.dims and "time" not in ds.coords:
        return {default_year if default_year is not None else 0: ds}
...
def make_total_panel(years_axis, series, deltas, eq_year, pct_area_noeq, glob_thresh, glob_thresh_area, subper, lons, lats):
    ...
    years = np.array(years_axis)
	    
    for (row,col), (v, which) in zip(positions, varpairs):
        ax = fig.add_subplot(gs[row, col])
        if v == "PCTAREA":
            ax.plot(years[1:], pct_area_noeq, marker="o", markersize=1, lw=0.6, color="black")
        ...
        if which != "Δ":
            y = series[v]
            ax.plot(years, y, marker="o", markersize=1, lw=0.6, color="black")
            if eq_year.get(v) is not None:
                ax.axvline(eq_year[v], ls=":", lw=0.8, color="gray")
                ax.text(0.98, 0.02, f"EqYr={eq_year[v]}", transform=ax.transAxes,va="bottom", ha="right", fontsize=8, 
            ...
            y = deltas[v]
            ax.plot(years[1:], y, marker="o", markersize=1, lw=0.6, color="black")
    ...
    for ax, data, ttl in [
        (ax_map1, last, f"ΔTOTECOSYSC Yr: {years_axis[-2]}–{years_axis[-1]}"),
        (ax_map2, prev, f"ΔTOTECOSYSC Yr: {years_axis[-3]}–{years_axis[-2]}")
    ]:
...
# Paths
base_spinup_dir = f"/p/scratch/cjibg36/jibg3674/eCLM_BGC_SPINUP/postAD"

# Discover run_* subperiod directories
run_pattern = re.compile(r"^run_(\d{8})_(\d{8})$")
cycle_dirs, subperiods = [], []
for folder in sorted(os.listdir(base_spinup_dir)):
    m = run_pattern.match(folder)
    if not m:
        continue
    sdate, edate = m.groups()
    syear, eyear = int(sdate[:4]), int(edate[:4])
    rundir = os.path.join(base_spinup_dir, folder)
    restart_file = os.path.join(rundir, f"{caseid}.clm2.r.{eyear:04d}-01-01-00000.nc")
    if os.path.exists(restart_file):
        cycle_dirs.append(rundir)
        subperiods.append((syear, eyear))
...
glob_thresh = {
    "TOTECOSYSC": 0.004,   # Pg C / yr
    "TOTSOMC":    0.004,   # Pg C / yr
    "TOTVEGC":    0.004,   # Pg C / yr
    "TLAI":       0.02,    # m2/m2 per yr (change in global mean)
    "GPP":        0.004,   # Pg C / yr^2 (change in PgC/yr per yr)
    "TWS":        0.001,   # m / yr
}
...
for ic, cdir in enumerate(cycle_dirs):
    syear, eyear = subperiods[ic]
    years = list(range(syear, eyear+1))
    print(f"[{ic+1}/{len(cycle_dirs)}] Processing {cdir} (SimYear: {syear}-{eyear})")

    ds = _open_cycle_dataset(cdir, years, variables)
    groups = _annual_means_by_year(ds, default_year=syear)
    start_year = years[0]
    ds_first = _get_last_year_annual(groups, start_year)

    # Compute scalars per variable
    for v in variables:
        if v not in ds_first.variables:
            raise KeyError(f"Variable {v} missing in dataset")
        val = _global_aggregate(v, ds_first[v], landarea)
        series[v].append(val)
        
    # Save field for TOTECOSYSC area-% and maps (keep in gC m^-2)
    fields_totecosysc.append(ds_first["TOTECOSYSC"].load())
    ds.close()
    del ds
    gc.collect
...
years_axis = [syear for (syear, eyear) in subperiods]
eq_year = {}
for v in variables:
    eq_cycle_idx = _equilibrium_cycle(series[v], glob_thresh[v], subper)
    if eq_cycle_idx is not None:
        eq_year[v] = years_axis[eq_cycle_idx]
    else:
        eq_year[v] = None
...
make_total_panel(years_axis, series, deltas, eq_year, pct_area_noeq, glob_thresh, glob_thresh_area, subper, lons, lats)
